...
export class Camera implements IObject {
    ...
    constructor() {
       ...
       // this.target.position  = Vector3.add(this.transform.position , new Vector3(this.transform.forward.x, this.transform.forward.y, this.transform.forward.z +5));
        this.target.position =  new Vector3(0, 0, 1);
        $WGPU.addObject(this);
    ...
    rotateAroundTarget(mouseX: number, mouseY: number){
        ...
        const yVec = vec3.rotateY(this.transform.position.toArray ,this.target.position.toArray, $TIME.deltaTime  )
       ...
       // const rot = Quaternion.fromToRotation(this.transform.forward.normalized, dir.normalized);
        const rot = Quaternion.lookRotation(dir);
        console.log(rot.getEulerAngles())
        this.transform.rotation = rot;
        console.log(this.transform.eulerAngles)
