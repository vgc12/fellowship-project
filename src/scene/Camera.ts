...
export class Camera implements IObject {
    ...
    viewMatrixLookingAt(target: Vector3, up = this.transform.up.toArray) {
        return mat4.lookAt(this._transform.position.toArray, target.toArray, up);
    }
    ...
    update() {
        ...
        const t = this.target ?? Vector3.add(this._transform.position, this._transform.forward.normalized);

        this._viewMatrix = this.viewMatrixLookingAt(t);
