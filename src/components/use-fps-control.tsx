...
export const useFpsControl = (initialValues: FPSValues) => {
    const fpsValues = useRef(initialValues);
    ...
    const run = () => {
        ...
        $WGPU.cameraController.orbitRotation.x = fpsValues.current.y * sensitivity;
        $WGPU.cameraController.orbitRotation.y = fpsValues.current.x * sensitivity;
    ...
    useEffect(() => {
        if (running) {
            fpsValues.current.x = $WGPU.cameraController.orbitRotation.y;
            fpsValues.current.y = $WGPU.cameraController.orbitRotation.x;
    ...
    const updateDirection = useCallback((direction: Direction, distance = 1) => {
        ...
        fpsValues.current = {
            x: prev.current.x + delta.x,
            y: prev.current.y + delta.y,

        };

        prev.current = fpsValues.current;

    }, []);


    return {orbitValues: fpsValues, onJoystickChange, setRunning};
};