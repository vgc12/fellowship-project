        ...
        <>
            {
                ...
                ['Position', 'Rotation'].map(label =>
                {
                    ...
                    return <VectorInputComponent label={label} key={label} numberLabels={['x','y','z']}
                                                 values={values.toArray} onChange={
                        (val, axis) =>
                        {
                            const transformKey = axis as 'x' | 'y' | 'z';
                            console.log(values, transformKey);
                            values[transformKey] = val;
                            console.log($WGPU.lights.find(l => l.guid === props.object.guid));
                        }
                    }/>

                })
            }
            <VectorInputComponent min={0} max={1} step={0.01} label={'Color'} numberLabels={['R', 'G', 'B']} values={props.object.color.toArray}
                                  onChange={
                                      (value, axis) =>
                                      {
